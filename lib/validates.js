...
let IsIdCard = (str) => {
  ...
  //   sex = '女';
  // }
  // // console.log(`出生年月日：${year} 年 ${month} 月 ${day} 日，性别：${sex}`)
  // let code = value.substr(0, 6); // 归属地区 code 码
  ...
  let y = c % 11,
    r = [1, 0, 10, 9, 8, 7, 6, 5, 4, 3, 2],
    l = value.substr(17, 1).toLowerCase();
...
// 身份证号码
// idCard: true
// idCard: [true, '身份证号码格式不正确']
export function idCard(val, rule) {
  let valid = true,
    tip = '身份证号码格式不正确';
  val = trim(val);
  if (val) {
